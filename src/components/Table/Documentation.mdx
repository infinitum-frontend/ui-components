...
2. Для описания рядов необходимо передать в проп <strong>rows</strong> массив с элементами для наполнения. Тип элемента должен быть аналогичен тому, что передан в <strong>columns</strong> Пример:
...
Также имеется возможность указанному ряду применить `style` и `className`. Для этого в модель данных нужно добавить className/style определенному ряду:
